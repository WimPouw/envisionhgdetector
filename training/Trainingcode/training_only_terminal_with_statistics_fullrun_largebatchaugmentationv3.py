...
class Config:
    ...
    npz_filename: str = "./training/bodylandmarks2.npz" # this is where the training data is stored (mediapipe holistic body from the training videos)
    seq_length: int = 25 # this sets the window size for the classifier, so 25 frames input (so the model needs 25 frames to make a prediction)
    ...
    weights_filename: str = f"./training/snelliusSAGAZHUBOTEDM3DMULTISIMOECOLANGAUGMENTEDv4.h5" # this contains the CNN model weights already trained with snellius, we will just go on here, but with augmentation
...
class EnhancedPreprocessing(layers.Layer):
    def __init__(
        ...
        drop_prob: float = 0.05,                              # Lower drop probability
        noise_stddev: float = 0.01,                           # Lower noise for normalized features
        scale_range: Tuple[float, float] = (0.97, 1.03),      # Smaller scale range
        mask_max_size: int = 3                                # Shorter mask length
    ) -> None:
    ...
    def random_rotation(self, features: tf.Tensor) -> tf.Tensor:
        """Apply random rotation only to specific spatial coordinates."""
        theta = tf.random.uniform([], self.rotation_range[0], self.rotation_range[1])
        c = tf.cos(theta)
        s = tf.sin(theta)
        rotation = tf.stack([[c, -s], [s, c]])
        
        # Get shape information
        batch_size = tf.shape(features)[0]
        time_steps = tf.shape(features)[1]
        
        # Only rotate nose x,y coordinates (features 4-5)
        nose_coords = features[:, :, 3:5]  # nose_x, nose_y
        nose_flat = tf.reshape(nose_coords, [-1, 2])
        rotated_nose = tf.matmul(nose_flat, rotation)
        rotated_nose = tf.reshape(rotated_nose, [batch_size, time_steps, 2])
        
        # Combine rotated coordinates with other features
        features_updated = features
        features_updated = tf.concat([
            features[:, :, :3],    # rotation features
            rotated_nose,          # rotated nose x,y
            features[:, :, 5:]     # remaining features
        ], axis=-1)
...
def train_and_save_weights(
    ...
    BATCH_SIZE = 1024  # Safe batch size for A100 with your model
